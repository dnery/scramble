...
void cube::bind(program *program) const
{
        ...
        if (glGetUniformLocation(program->get(), "material.texture_specular1") != -1) {

                glActiveTexture(GL_TEXTURE2);
                glBindTexture(GL_TEXTURE_2D, specular);
                glUniform1i(program->uniform("material.texture_specular1"), 2);
        }
        ...
        texmap(nullptr)
{
        ...
        // 4.1 Make & bind texture information
        glGenTextures(1, &texture1);
        texmap = SOIL_load_image(resource_path("textures/wooden-floor.jpg").c_str(),
                                 &texw, &texh, 0, SOIL_LOAD_RGB);
        glBindTexture(GL_TEXTURE_2D, texture1);
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, texw, texh, 0,
                     GL_RGB, GL_UNSIGNED_BYTE, texmap);
        ...
        // 4.2 Make & bind texture information
        glGenTextures(1, &specular1);
        texmap = SOIL_load_image(resource_path("textures/wooden-floor-specular").c_str(),
                                 &texw, &texh, 0, SOIL_LOAD_RGB);
...
{
        glDeleteTextures(1, &texture1);
        glDeleteTextures(1, &specular1);
        glDeleteVertexArrays(1, &vao);
        glDeleteBuffers(1, &vbo);
}

void plane::bind(program *program) const
{
        /*
         * Without this check, instantiating a non-textured plane would not be
         * possible using the current framework. It's repeated for every
         * possible texture uniform available in the shader.
         */
        if (glGetUniformLocation(program->get(), "material.texture_diffuse1") != -1) {

                glActiveTexture(GL_TEXTURE0);
                glBindTexture(GL_TEXTURE_2D, texture1);
                glUniform1i(program->uniform("material.texture_diffuse1"), 0);
        }
        ...
        if (glGetUniformLocation(program->get(), "material.texture_specular1") != -1) {
                ...
                glActiveTexture(GL_TEXTURE2);
                glBindTexture(GL_TEXTURE_2D, specular1);
                glUniform1i(program->uniform("material.texture_specular1"), 1);
        }
